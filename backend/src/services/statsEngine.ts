...
export interface SignalMetrics {
  a: number; // drug + event
  b: number; // drug + other event
  c: number; // other drug + event
  d: number; // other drug + other event
  prr: number;
  ror: number;
  chiSquare: number;
  ciLower: number;
  ...
  signalScore: number;
  strength: 'Strong' | 'Moderate' | 'Weak' | 'No Signal';
}
...
export function calculateMetrics(
  ...
  if (isNaN(prr) || !isFinite(prr)) prr = 0;

  // 2. Reporting Odds Ratio (ROR)
  // ROR = (a * d) / (b * c)
  let ror = (b * c) === 0 ? 0 : (a * d) / (b * c);
  if (isNaN(ror) || !isFinite(ror)) ror = 0;
  ...
  if (isNaN(chiSquare) || !isFinite(chiSquare)) chiSquare = 0;
  ...
  const se = Math.sqrt(
    1 / aCorr - 1 / (aCorr + bCorr) + 1 / cCorr - 1 / (cCorr + dCorr)
  );
  ...
  if (isNaN(informationComponent) || !isFinite(informationComponent)) informationComponent = 0;
  ...
  let strength: 'Strong' | 'Moderate' | 'Weak' | 'No Signal' = 'No Signal';
  if (isStrong) {
    strength = 'Strong';
  } else if (isModerate) {
    strength = 'Moderate';
