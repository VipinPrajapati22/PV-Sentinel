...
exports.escalateSignal = escalateSignal;
...
async function getRisks(req, res) {
    try {
        const risks = await prisma_1.default.riskRegister.findMany({
            include: {
                signal: {
                    include: { drug: true }
                },
 ...
 */
async function escalateSignal(req, res) {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        const { signalId, riskCategory, severityScore, likelihoodScore, capaDescription, ownerId } = req.body;
        ...
        // Verify owner exists
        const owner = await prisma_1.default.user.findUnique({ where: { id: ownerIdText } });
        if (!owner) {
...
async function updateRisk(req, res) {
    try {
        ...
        const { riskCategory, severityScore, likelihoodScore, capaDescription, capaStatus, ownerId } = req.body;
        const existingRisk = await prisma_1.default.riskRegister.findUnique({ where: { id } });
        if (!existingRisk) {
        ...
        const riskScore = severityVal * likelihoodVal;
        const updatedRisk = await prisma_1.default.riskRegister.update({
            where: { id },
            data: {
...
async function updateCapa(req, res) {
    try {
        ...
        const existingCapa = await prisma_1.default.capaTracking.findUnique({ where: { id } });
        if (!existingCapa) {
            return res.status(404).json({ error: 'CAPA tracking record not found' });
        }
        const completedDate = completed === true ? new Date() : null;
        const updatedCapa = await prisma_1.default.capaTracking.update({
            where: { id },
            data: {
                actionTaken,
                status,
        ...
        if (status === 'Closed') {
            const remainingOpenCapas = await prisma_1.default.capaTracking.count({
                where: {
            ...
            if (remainingOpenCapas === 0) {
                await prisma_1.default.riskRegister.update({
